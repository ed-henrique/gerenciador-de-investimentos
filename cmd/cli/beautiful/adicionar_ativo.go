...
type AdicionarAtivoTelaModelo struct {
	focusIndex int
...
func AdicionarAtivoTela(c *internal.Carteira) AdicionarAtivoTelaModelo {
	a := AdicionarAtivoTelaModelo{
		carteira: c,
...
func (a *AdicionarAtivoTelaModelo) Init() tea.Cmd {
	return textinput.Blink
}

func (a *AdicionarAtivoTelaModelo) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	switch msg := msg.(type) {
	case tea.KeyMsg:
		switch msg.String() {
		case "ctrl+c":
		...
		case "tab", "shift+tab", "enter", "up", "down":
			...
			if s == "enter" && a.focusIndex == len(a.inputs) {
				...
				valorUnitario, err := strconv.ParseFloat(a.inputs[2].Value(), 64)

				if err != nil {
					a.erro = "Erro: O valor unitário inserido deve ser um número inteiro ou de ponto flutuante."
					return a, nil
				}

				err = a.carteira.AdicionarAtivo(internal.Ativo{
					Codigo:        codigo,
...
func (a *AdicionarAtivoTelaModelo) updateInputs(msg tea.Msg) tea.Cmd {
	cmds := make([]tea.Cmd, len(a.inputs))
...
func (a *AdicionarAtivoTelaModelo) View() string {
	var s strings.Builder

	fmt.Fprintf(&s, "%s\n\n", aplicarTitulo("Adicionar Ativo"))
	...
	botaoAdicionar := &botaoAdicionarNaoFocado
	botaoVoltar := &botaoVoltarNaoFocado

	if a.focusIndex == len(a.inputs) {
		botaoAdicionar = &botaoAdicionarFocado
	}
	if a.focusIndex == len(a.inputs)+1 {
		botaoVoltar = &botaoVoltarFocado
	}

	fmt.Fprintf(&s, "\n\n%s\n%s\n\n", *botaoAdicionar, *botaoVoltar)
